...
Calling the stochastic gradient descent method:
...
The above plot shows that a linear model (polynomial model of degree 1) is an optimal model for the given data, since its cross validation error is minimal.
...
The graph clearly shows that the MSE is almost the same for both Batch Gradient descent and Stochastic MSE. So for the prediction of house prices we can safely use Stochastic gradient descent.
